...
export class WomensSunglassesCatalog
{
    ...
    applyInStockFilter()
    {
        ...
        //cy.get('.v_quickship_uFilter > h4 > .switch > .slider').if().click() //Enable toggle
    ...
    applyBrandFilters()
    {
        const random  = reuseableCode.getRandomNumber(0,100) 
    ...
    applySizeFilter()
    {
        cy.get('select[name="data[[Filter]][Lens Width]"]').should('exist').select('47') //Select Lense Width
        cy.url().should('include','/lens-width:47/') //validate URL
        cy.wait(3000)
        cy.get('.clear_all_selected_facets').should('contain.text','Reset All Filters').click() //Reset All Filters
        cy.wait(3000)
        cy.get('select[name="data[[Filter]][Bridge Width]"]').should('exist').select('25') //Select Bridge Width
        cy.url().should('include','/bridge-width:25/')//validate URL
        cy.get('.clear_all_selected_facets').should('contain.text','Reset All Filters').click() //Reset All Filters
        cy.wait(3000)
        cy.get('select[name="data[[Filter]][Arm Length]"]').should('exist').select('135') //Select Arm Length
        cy.url().should('include','/arm-length:135/')//validate URL
    }
